...
public:
  ...
  static const Eigen::Matrix4d create_homogeneous_matrix(const Eigen::Vector3d &position, const Eigen::Matrix3d &orientation); // create homogeneous matrix from position and orientation
  ...
  static const Eigen::MatrixXd p2p(const Eigen::VectorXd &qEs, const Eigen::VectorXd &qEf); // get trajectory in order to reach end-effector position and orientation
  ...
  // temp
  static const Eigen::VectorXd best_angles(const Eigen::VectorXd &actual, const Eigen::MatrixXd &possible); // get best angles for p2p trajectory
...
private:
  static const Eigen::Matrix4d compute_matrix(double theta, short frame); // return matrix based on theta and frame
  static const bool check_row(const Eigen::VectorXd &vec);                // check if matrix contains NaN values
};