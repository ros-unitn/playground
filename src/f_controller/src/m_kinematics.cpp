...
const Eigen::VectorXd K_UR5::a = (Eigen::VectorXd() << 0, -0.425, 0.39225, 0, 0, 0).finished();
const Eigen::VectorXd K_UR5::d = (Eigen::VectorXd() << 0.089159, 0, 0, 0.10915, 0.09465, 0.0823).finished();
const Eigen::VectorXd K_UR5::alpha = (Eigen::VectorXd() << 0, M_PI_2, 0, 0, M_PI_2, -M_PI_2).finished();

Eigen::Matrix3d K_UR5::eul2rotm(const Eigen::Vector3d &eul) {
...
Eigen::Vector3d K_UR5::rotm2eul(const Eigen::Matrix3d &rotm) {
  return rotm.eulerAngles(2, 1, 0);
}

Eigen::Matrix4d K_UR5::computeMatrix(double theta, short frame) {
  return (Eigen::Matrix4d() << cos(theta), -sin(theta), 0, a(frame),
...
Eigen::MatrixXd K_UR5::p2p(const Eigen::VectorXd &qEs, const Eigen::Vector3d &xEf, const Eigen::Vector3d &phiEf) {

  Eigen::VectorXd qEf = ik(createHomogeneousMatrix(xEf, eul2rotm(phiEf))).row(0);

  Eigen::MatrixXd A(6, 6);

  Eigen::Matrix4d M = (Eigen::Matrix4d() << 1, minT, minT * minT, minT * minT * minT, minT * minT * minT * minT, minT * minT * minT * minT * minT,
...
Eigen::Matrix4d K_UR5::createHomogeneousMatrix(const Eigen::Vector3d &position, const Eigen::Matrix3d &orientation) {
